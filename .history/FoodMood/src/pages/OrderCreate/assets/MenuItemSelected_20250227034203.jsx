...
const MenuItemSelected = ({
    ...
    useEffect(() => {
        ...
        // คำนวณวัตถุดิบคงเหลือใหม่โดยหักลบวัตถุดิบที่ใช้ไป
        const remain = selectedItem.sizePrices[0].ingredients.map((acc) => {
            ...
            return ingredient
                ? { ...ingredient, remain: ingredient.remain - (acc.newIngredientQty * quantity) } // หักลบตามจำนวนที่ใช้
                : { name: acc.newIngredient, remain: 0 }; // ถ้าไม่มีข้อมูลให้แสดงเป็น 0
        });
    
        setIngredientRemainList(remain);
    }, [selectedItem, quantity, ingredientRemain]);
