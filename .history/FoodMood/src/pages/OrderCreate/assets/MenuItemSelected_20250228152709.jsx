    ...
    return (
        <>
            ...
            <Grid container sx={{ borderBottom: "2px solid #ddd", marginBottom: "8px", paddingBottom: "8px" }}>
                ...
                <Grid item xs={1}>
                {formMode === "Edit" ? <></> : <CloseIcon style={{ fontSize: 18, color: "#ff4b4b", cursor: "pointer" }} onClick={() => onClose()} />}
                </Grid>
            ...
            <Grid sx={{ flexGrow: 1 }}>
                                ...
                                <Grid key={index} item xs={12} style={{ paddingLeft: '10%' }}>
                                    <Grid container justifyContent={'space-between'}>
                                        ...
                                        <Grid item xs={5} display={'flex'} justifyContent={'flex-end'} style={{ paddingRight: '5%' }}>
                                            <p>{(acc.newIngredientQty * quantity).toFixed(2)}/</p><p style={{ color: (acc.newIngredientQty * quantity) > ingredientRemainList[index].remain ? "red" : "black" }}>{ingredientRemainList[index].remain.toFixed(2)}</p>
                                        </Grid>
                                        <Grid item xs={1}>
                                            <p>{acc.unit}</p>
                                        </Grid>
                ...
                <Grid container spacing={1} sx={{ display: "flex", alignItems: "center", marginBottom: "8px", paddingBottom: "8px", borderBottom: "1px solid #ddd" }}>
                    ...
                    <Grid item xs={10} sx={{display: "flex",alignItems: "center",justifyContent: "flex-end",}}>
                        <button  onClick={handleQuantityDecrease} className="quantity-left-btn" disabled={quantity === 1}> - </button>
                        <span style={{ display: "flex", justifyContent: "center", width: "30px" }}>{quantity}</span>
            ...
            {/* Action Buttons */}
            <Grid display="flex" justifyContent={formMode === "Edit" ? "flex-end" : "space-between"}  borderTop="1px solid #ddd" sx={{ marginTop: "auto", paddingTop: "8px" }}>
                {formMode === "Edit" ? <></> : <button className='red-button' onClick={onClose}>ยกเลิก</button>}
                <button className='blue-button' onClick={handleConfirmAdd}>{formMode === "Edit" ? "อัปเดตรายการ" : "เพิ่มเมนูใหม่"}</button>
            </Grid>
        </>
    );
};
