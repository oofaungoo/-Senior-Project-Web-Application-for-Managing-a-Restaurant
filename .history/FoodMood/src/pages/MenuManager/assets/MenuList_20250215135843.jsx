                        ...
                        <Grid item xs={6} md={4} lg={2} key={item._id} minHeight={200} onClick={() => onMenuClick(item)} >
                                ...
                                <Button style={{height:"100%",padding:0,margin:0,display:"flex"}} >
                                    <Grid container  direction="column" style={{padding:10,height:"100%"}}>
                                        <img style={{borderRadius:5,marginBottom:10,height:"5rem", width:"100%",justifyContent={"center"}} src={item.image || Food} alt={item.name} />
                                        <p style={{fontSize:13,color:'#00424'}}>{item.name}</p>
                                        <p style={{fontSize:13,color:'#777777' }}>{item.sizePrices.map(size => size.size).join('/')}</p>
                                    </Grid>
                                </Button>
                                </Paper>
                        </Grid>
