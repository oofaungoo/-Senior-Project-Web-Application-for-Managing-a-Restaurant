...
const AddNewMenu = ({ form, dataToEdit, onCancel, setShowItemDetail }) => {
    ...
    useEffect(() => {
        if (menuData?.image) {  // เช็คว่ามีรูปที่บันทึกไว้ไหม
            setImageUrl(menuData.image);
        }
    }, [menuData]); // จะทำงานเมื่อ menuData เปลี่ยนแปลง
                ...
                <Stack sx={{ minHeight: "100%", display: "flex" }} onSubmit={(e) => e.preventDefault()}>
                    ...
                    <Grid sx={{ flexGrow: 1 }}>
                        ...
                        <Grid>
                            ...
                            <Grid container>  {/* spacing = ระยะห่างระหว่างกล่อง */}
                                {sizePrices.map((item, index) => (
                                    <Grid item xs="auto" key={index}> {/* xs="auto" ทำให้ขนาดปรับตามเนื้อหา */}
                                        <div
