...
const MenuItemDetail = ({ selectedItem, onEdit, handleClickConfirm, onClose }) => {
    ...
    return (
        <Stack sx={{ minHeight: "100%", display: "flex" }}>
            ...
            <Grid sx={{flexGrow:1}}>
                {/* 1. Image */}
                <img src={selectedItem.image || Food} alt={selectedItem.name} style={{ width: "100%", height: "160px", objectFit: "cover", borderRadius: "5px" }}/>
                

                {/* 2. Size: can select size to see price & ingredient details */}
                <Stack direction="row" spacing={1} flexWrap="wrap" rowGap={1} paddingBottom={"8px"} marginBottom={"8px"} borderBottom="1px solid #ddd">
                ...
                <Grid sx={{ borderBottom: "1px solid #ddd", paddingBottom: "8px", marginBottom: "8px" }}>
                    <>
                        {selectedSize && (
                            <>
            ...
            {/* Action Buttons */}
            <Grid display="flex" justifyContent="space-between" borderTop="1px solid #ddd" sx={{ flexGrow: 1, paddingTop: "8px" }}>
