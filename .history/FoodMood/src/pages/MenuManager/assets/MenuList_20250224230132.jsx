    ...
    return (
        <>
            ...
            <Grid container spacing={1}>
                {/* กล่องเพิ่มเมนูใหม่ */}
                <Grid item xs={6} md={4} lg={2} height={190}>
                ...
                {filteredMenuItems.map((item) => (
                    <Grid item xs={6} md={4} lg={2} key={item._id} height={190} onClick={() => onMenuClick(item)}>
                        <Paper elevation={2} style={{ height: "100%", display: "flex", flexDirection: "column" }}>
                            <Button style={{ height: "100%", padding: 0, margin: 0, display: "flex", flexDirection: "column" }}>
                                <Grid container direction="column" style={{ height: "100%"}}>
                                    <img
                                        style={{ borderRadius: 5, marginBottom: 10, width: "100%", height:"4rem", objectFit: "cover", }}
                                        src={item.image || Food}
                                        alt={item.name}
                                    />
