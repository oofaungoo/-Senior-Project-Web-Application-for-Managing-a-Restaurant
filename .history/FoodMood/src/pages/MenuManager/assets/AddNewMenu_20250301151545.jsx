...
const AddNewMenu = ({ form, dataToEdit, onCancel, setShowItemDetail }) => {
    ...
    const [imageUrl, setImageUrl] = useState(dataToEdit?.image || null);
    ...
    const handleAddIngredient = (value) => {
        ...
        const intoSizePrices = sizePrices.map((item) => {
            if (item.size === value.sizeName) {
                return { ...item, ingredients: value.ingredients,sizeName: value.sizeName, price: value.price};
            }
            return item;
        });
    ...
    const handleSave = () => {
        if (sizePrices.length === 0) {
            Swal.fire({
                icon: "warning",
                title: "กรุณาเพิ่มขนาดและราคา",
                text: "ต้องมีขนาดและราคาขั้นต่ำอย่างน้อย 1 รายการ",
                confirmButtonColor: "#62c965",
            });
            return;
        }
        ...
        if (form === "Edit") {
            axios.put(`http://localhost:5000/api/foods/${dataToEdit._id}`, menuData)
                .then(() => {
                    Swal.fire({
                        icon: "success",
                        title: "บันทึกสำเร็จ!",
            ...
            axios.post('http://localhost:5000/api/foods', menuData)
                .then(() => {
                    Swal.fire({
                        icon: "success",
            ...
            {!ingredientScreen ? (
                <Stack sx={{ minHeight: "100%", display: "flex" }} onSubmit={(e) => e.preventDefault()}>
                    ...
                    <Grid sx={{ flexGrow: 1 }}>
                        ...
                        {/* 3. Image url upload */}
                        <Grid direction="column" style={{ marginTop: "10px", cursor: "pointer" }} onClick={handleImageUpload}>
                            {imageUrl ? (
                                <img src={imageUrl} alt="Uploaded" style={{ width: "100%", height: "160px", objectFit: "cover", borderRadius: "5px" }} />
                            ) : (
                                <Box sx={{width: '100%', height: '160px', borderRadius: '5px', color: '#777', backgroundColor: '#eee', display: 'flex', flexDirection: 'column', justifyContent: 'center',alignItems: 'center'}}>
