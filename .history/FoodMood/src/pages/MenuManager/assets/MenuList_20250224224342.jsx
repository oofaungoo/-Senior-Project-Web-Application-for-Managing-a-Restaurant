...
{filteredMenuItems.map((item) => (
    <Grid item xs={6} md={4} lg={2} key={item._id} minHeight={200} onClick={() => onMenuClick(item)}>
        <Paper elevation={2} style={{ height: "100%", display: "flex", flexDirection: "column" }}>
            <Button style={{ height: "100%", padding: 0, margin: 0, display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                <Grid container direction="column" style={{ padding: 10, height: "100%" }}>
                    <img
                        ...
                        alt={item.name}
                    />
                    <p style={{ fontSize: 18, fontWeight: 4, color: '#000000', textAlign: "left", width: "100%" }}>
                        {item.name}
                    </p>
                    <p style={{ fontSize: 15, fontWeight: 4, color: '#777777', textAlign: "left", width: "100%" }}>
